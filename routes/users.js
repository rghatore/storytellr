...
module.exports = (database) => {
  ...
  router.post("/login", (req, res) => {
    ...
    login(email, password, database)
      .then((user) => {
        if (user === "That's not your password!") {
          // console.log({error: "User not found! Please register."});
          // console.log(user)
          res.send({ error: "That's not your password!" });
          return;
        } else if (user === "User not found! Please register") {
  ...
  router.get("/:username", (req, res) => {
    ...
    // .then(data => {
    database
      .getAllStories({ user_name: username, search: null })
      .then((stories) => {
        if (!stories) {
